...
import org.apache.openwhisk.core.containerpool.SttpContainerClient
...
/**
 * Unit tests for SttpContainerClient which communicate with containers.
 */
@RunWith(classOf[JUnitRunner])
class SttpContainerClientTests
    extends FlatSpec
    with Matchers
    with BeforeAndAfter
  ...
  behavior of "SttpContainerClientTests"
  ...
  it should "not truncate responses within limit" in {
    ...
    Seq(true, false).foreach { success =>
      Seq(null, "", "abc", """{"a":"B"}""", """["a", "b"]""").foreach { r =>
        ...
        testResponse = r
        val result = Await.result(connection.post("/init", JsObject.empty, retry = true), 10.seconds)
        println(s"result: ${result.right.get.entity}")
        result shouldBe Right {
