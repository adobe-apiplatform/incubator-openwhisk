...
class ContainerPoolTests
  ...
  it should "reuse a warm container" in within(timeout) {
    ...
    val pool =
      system.actorOf(ContainerPool.props(instanceId, factory, poolConfig(MemoryLimit.STD_MEMORY * 4), feed.ref))
  ...
  it should "remove a container to make space in the pool if it is already full and a different action arrives" in within(
    ...
    val pool = system.actorOf(
      ContainerPool
        .props(instanceId, factory, poolConfig(MemoryLimit.STD_MEMORY), feed.ref, resMgrConfig = resMgrConfig()))
    pool ! runMessage
    containers(0).expectMsg(runMessage)
    containers(0).send(pool, NeedWork(warmedData(runMessage)))
  ...
  it should "remove several containers to make space in the pool if it is already full and a different large action arrives" in within(
    ...
    val pool = system.actorOf(
      ContainerPool.props(instanceId, factory, poolConfig(512.MB), feed.ref, resMgrConfig = resMgrConfig()))
    pool ! runMessage
    containers(0).expectMsg(runMessage)
  ...
  it should "cache a container if there is still space in the pool" in within(timeout) {
    ...
    val pool =
      system.actorOf(
        ContainerPool
          .props(instanceId, factory, poolConfig(MemoryLimit.STD_MEMORY * 2), feed.ref, resMgrConfig = resMgrConfig()))
  ...
  it should "reschedule job when container is removed prematurely without sending message to feed" in within(timeout) {
    ...
    // a pool with only 1 slot
    val pool = system.actorOf(ContainerPool.props(instanceId, factory, poolConfig(MemoryLimit.STD_MEMORY), feed.ref))
    pool ! runMessage
    containers(0).expectMsg(runMessage)
  ...
  it should "create prewarmed containers on startup" in within(timeout) {
    ...
    val pool =
      system.actorOf(
        ContainerPool
          .props(instanceId, factory, poolConfig(memoryLimit), feed.ref, List(PrewarmingConfig(1, exec, memoryLimit))))
